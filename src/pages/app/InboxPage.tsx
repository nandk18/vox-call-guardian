        ...
        <div className="px-4 pt-3 space-y-2">
          {daysLeft !== null && (
            <div className="flex items-center gap-3 p-3 rounded-xl bg-teal-500/10 border border-teal-500/20">
              ...
              <div className="flex-1">
                <p className="text-xs font-semibold text-teal-300">You're on Free Trial</p>
                <p className="text-[11px] text-teal-400/70">
                  Expires on {trialEndsAt!.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                </p>
              </div>
            </div>
          )}
          {!agent?.phone_number && (
            <button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl bg-orange-500/10 border border-orange-500/20 text-left"
            >
              <div className="w-8 h-8 rounded-full bg-orange-500/20 flex items-center justify-center text-orange-400 text-sm">📱</div>
              <div className="flex-1">
                <p className="text-xs font-semibold text-orange-300">{phoneNumber}</p>
                <p className="text-[11px] text-orange-400/70">Call forwarding not set up yet</p>
              </div>
          ...
          <div className="p-4 md:p-6 space-y-4 max-w-2xl mx-auto w-full">
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-secondary text-foreground flex items-center gap-1">
                  <Clock className="w-3 h-3" /> {formatDuration(selectedCall.duration_secs)}
                </span>
            ...
            <div className="bg-card rounded-2xl border border-border p-5 border-l-4 border-l-primary space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                ...
                <InfoRow icon="⏰" label="Preferred Time" value={null} />
                <InfoRow icon="📞" label="Best Time to Call" value={null} />
              </div>
            </div>
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-3">
              <div className="flex flex-wrap gap-2">
                <a
                  href={`tel:${selectedCall.caller_number}`}
                ...
                <button className="flex items-center gap-2 px-4 py-2.5 bg-transparent border border-border text-muted-foreground rounded-xl text-sm hover:bg-secondary transition-colors">
                  <RotateCcw className="w-4 h-4" /> Resend Summary
                </button>
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <button className="flex items-center gap-2 px-4 py-2.5 bg-transparent border border-destructive/30 text-destructive rounded-xl text-sm hover:bg-destructive/10 transition-colors">
                      <Trash2 className="w-4 h-4" /> Delete Call
                    </button>
                  </AlertDialogTrigger>
