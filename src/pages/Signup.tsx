...
const Signup = () => {
  ...
  const handleOtpChange = (index: number, value: string) => {
    if (value.length > 1) {
      ...
      document.getElementById(`signup-otp-${nextIdx}`)?.focus();
      if (newOtp.every((d) => d)) setTimeout(() => handleAutoVerify(newOtp), 100);
      return;
    }
    const newOtp = [...otp];
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        data: { business_name: emailBizName.trim() },
      },
    });
    setLoading(false);
    if (error) setError(error.message);
        ...
        {tab === "phone" ? (
          ...
          emailSent ? (
            ...
            <form onSubmit={handleMagicLink} className="space-y-4">
              <input type="text" required placeholder="Business Name" value={emailBizName} onChange={(e) => setEmailBizName(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-background border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 text-base" />
              <input type="email" required placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-background border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 text-base" />
              {error && <p className="text-destructive text-sm">{error}</p>}
